...
import kotlinx.android.synthetic.main.activity_mobile_recharge_receipt.*
import java.io.File
import java.io.FileOutputStream

class MobileRechargeReceipt : BaseActivity() {
    ...
    private var mobileRechargeResponse: MobileRechargeResponse? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_mobile_recharge_receipt)
        val type = object : TypeToken<MobileRechargeResponse>() {}.type
        mobileRechargeResponse = EasyPaisaApp.getGson().fromJson<MobileRechargeResponse>(
            intent.getStringExtra(Constants.UI.MOBILE_RESPONSE),
            type
        )
    ...
    private fun fillAePSTxnDetails() {
        when (mobileRechargeResponse?.status) {
        ...
        tvTxnAmount.text = if (mobileRechargeResponse?.maount!!.isNotEmpty()) {
            Utils.formatAmount(mobileRechargeResponse?.maount!!.toDouble())
        } else {
            "NA"
        }
        tvTxnMessage.text = mobileRechargeResponse?.message
        tvTxnOrderId.text =
            if (mobileRechargeResponse?.txnid!!.isNotEmpty()) mobileRechargeResponse?.txnid else "NA"
        tvTxnMobileNo.text =
            if (mobileRechargeResponse?.number!!.isNotEmpty()) mobileRechargeResponse?.number else "NA"
        tvTxnRefNo.text =
            if (mobileRechargeResponse?.txnid!!.isNotEmpty()) mobileRechargeResponse?.txnid else "NA"
        tvTxnProviderName.text =
            if (mobileRechargeResponse?.providername!!.isNotEmpty()) mobileRechargeResponse?.providername else "NA"
        tvTxnDate.text =
            if (mobileRechargeResponse?.date!!.isNotEmpty()) mobileRechargeResponse?.date else "NA"
    }

    override fun getViewActivity(): Activity {
        return this@MobileRechargeReceipt
    }
    ...
    fun store(bm: Bitmap):File? {
        ...
        val folder = Utils.getExternalFilesAccessDir(this@MobileRechargeReceipt, Constants.MediaType.IMAGE)
        if (!folder.exists())
            folder.mkdirs()
